        ...
        var directive = {
            ...
            link: function(vm, element, attrs) {
                vm.$watch(attrs.ngModel, function(cases) {
                    ...
                    if (document.getElementById('bookcases')) {
                        document.getElementById('bookcases').removeEventListener('keypress', vm.zoomListener);
                        document.getElementById('bookcases').addEventListener('keypress', vm.zoomListener);
                    }
                }, true);
                ...
                vm.bookLocations = {};
                vm.getBookLocation = function(idx) {
                    return bookLocations[idx];
                };
                vm.drawShelf = function() {
                    ...
                    for (var c in vm.cases) {
                        ...
                        for (var s in vm.cases[c].shelves) {
                            vm.cases[c].shelves[s].width *= vm.zoom;
                            vm.cases[c].shelves[s].height *= vm.zoom;
                            if (vm.cases[c].shelves[s].width > maxWidth) {
                                maxWidth = vm.cases[c].shelves[s].width;
                            }
                        ...
                        caseHeights.push(h);
                        x += vm.cases[c].spacerheight + maxWidth + vm.cases[c].spacerheight + margin;
                        width = x;
                    }
                    ...
                    vm.cases.forEach(function(_, c) {
                        ...
                        canvas.width = vm.cases[c].spacerheight * 2;
                        for (var s in vm.cases[c].shelves) {
                            if (vm.cases[c].spacerheight * 2 + vm.cases[c].shelves[s].width > canvas.width) {
                                canvas.width = vm.cases[c].spacerheight * 2 + vm.cases[c].shelves[s].width;
                            }
                        }
                        ...
                        for (var s in vm.cases[c].shelves) {
                            var x = 0;
                            if (vm.cases[c].shelves[s].alignment == "right") {
                                x = canvas.width - vm.cases[c].spacerheight*2 - vm.cases[c].shelves[s].width;
                            }
                            var wood = document.getElementById('wood');
                            if (wood) {
                                ctx.drawImage(wood, x, y, vm.cases[c].shelves[s].width, caseHeights[c]);
                            }
                            ...
                            ctx.fillRect(x + vm.cases[c].shelves[s].width, y, vm.cases[c].spacerheight, vm.cases[c].spacerheight + vm.cases[c].shelves[s].height);
                            ctx.fillRect(x + vm.cases[c].spacerheight, y, vm.cases[c].shelves[s].width, vm.cases[c].spacerheight);
                            for (var b in vm.cases[c].shelves[s].books) {
                                ...
                                vm.bookLocations[vm.cases[c].shelves[s].books[b].id] = [ix + 1 * vm.zoom, y - bookheight + vm.cases[c].shelves[s].height + vm.cases[c].spacerheight + 1 * vm.zoom, bookwidth - 2 * vm.zoom, bookheight - 2 * vm.zoom];
                                ctx.save();
                            ...
                            ctx.fillRect(x, y, vm.cases[c].shelves[s].width + vm.cases[c].spacerheight, vm.cases[c].spacerheight);
                        }
                        y = 0;
                        x += vm.cases[c].spacerheight + vm.cases[c].width + vm.cases[c].spacerheight;
                    });
                    document.getElementsByClassName('bookcases')[0].scrollTo(0, height);
                };
            }
        };
        return directive;
